...
namespace util
{
  ...
  template <typename DType>
  bool check_result_sddmm(int nnz, DType *C, DType *C_ref)
  {
    bool passed = true;
    for (int64_t n = 0; n < nnz; ++n)
    {
      ...
      DType c_ref = C_ref[n];
      // printf(
      //     "Wrong result: nnz = %ld, result = %lf, reference = %lf.\n",
      //     n, c, c_ref);
      if (fabs(c - c_ref) > 1e-2 * fabs(c_ref))
      {
        printf(
  ...
  template <typename Index, typename DType>
  void sddmm_reference_host(int M,       // number of A-rows
                            int N,       // number of A-cols
                            int feature, // number of B_columns
                            Index *csr_indptr, Index *csr_indices,
                            DType *csr_values, // three arrays of A's CSR format
                            DType *B1,         // assume row-major
                            DType *B2,
                            DType *C_values) // assume row-major
  {
    for (int64_t i = 0; i < M; i++)
    {
      Index begin = csr_indptr[i];
      ...
      {
        for (Index k = 0; k < feature; k++)
        {
          C_values[j] += B1[i * feature + k] * B2[csr_indices[j] * feature + k]; // B2转置过
          // C_values[j] = B1[i * feature + k] * B2[k * N + j]; // B2未转置
        }
        C_values[j] *= csr_values[j];
      }
    }
  }
