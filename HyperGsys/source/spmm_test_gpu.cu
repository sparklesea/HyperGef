...
int main(int argc, char **argv)
{
  ...
  std::fstream fs;
  fs.open("gpu_result.csv", std::ios::app | std::ios::in | std::ios::out);
  ...
  balancer<Index, DType, balan_met::spmm_edge_group> balan_edgegroup(max_load, H, H_T);
  ...
  //         feature_size, H, H_T, balan_edgegroup, in_feature, tmp_feature, out_feature, out_ref))
    TwostepSpMM_check<Index, DType, spmm_kernel_met::edge_group, balan_met::spmm_edge_group>(
        feature_size, H, H_T, balan_edgegroup, in_feature, tmp_feature, out_feature, out_ref);

    TwostepSpMM_test<Index, DType, spmm_kernel_met::edge_group, balan_met::spmm_edge_group>(
